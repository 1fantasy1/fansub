...
fansub 是一个一键交互式代理节点部署脚本（"小钢炮脚本"），基于 **Sing-box + Xray** 双内核自动分配。支持在 Linux VPS 上部署多种代理协议节点。
